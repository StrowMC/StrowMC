...
public class ModListener implements Listener {
    ...
    public ModListener(MessageService messageService) {
    ...
    public void onInteract(PlayerInteractEvent e) {
        ...
        if (e.getPlayer().getItemInHand().getType().equals(Material.RECORD_3)
            ...
            messageService.sendMessage(e.getPlayer().getUniqueId(), "%s §cTu as été téléporté à %s.", ModerationModule.PREFIX, player.getName());
        }
            ...
            if (e.getPlayer().getItemInHand().getData().getData() == 10) {
                new BukkitRunnable() {
                    ...
                    public void run() {
                        ...
                        messageService.sendMessage(e.getPlayer().getUniqueId(), "%s §cLe mode vanish à été activé, tu es maintenant invisible par tous les joueurs.", ModerationModule.PREFIX);
                    }
                ...
                new BukkitRunnable() {
                    ...
                    public void run() {
                        ...
                        e.getPlayer().getInventory().setItem(3, invisibility);
                        messageService.sendMessage(e.getPlayer().getUniqueId(), "%s §cLe mode vanish à été désactivé, tu es maintenant visible par tous les joueurs.", ModerationModule.PREFIX);
                    }
                }.runTaskLater(JavaPlugin.getPlugin(StrowPlugin.class), 1);
            }
        }
            ...
            new BukkitRunnable() {
                ...
                public void run() {
                    if (ModerationModule.isFreezed(target.getUniqueId())) {
                        ...
                        messageService.sendMessage(target.getUniqueId(), "%s §cTu as été unfreeze, tu peux donc continuer à jouer", ModerationModule.PREFIX);
                        messageService.sendMessage(e.getPlayer().getUniqueId(), "%s §cTu as unfreeze le joueur %s", ModerationModule.PREFIX, target.getName());
                        ...
                        messageService.sendMessage(e.getPlayer().getUniqueId(), "%s, §cTu as freeze le joueur %s", ModerationModule.PREFIX, target.getName());
                        target.sendMessage(" ");
                        target.sendMessage(" ");
