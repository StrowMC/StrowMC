...
public class ArmorListener implements Listener {
	...
	public void playerDeathEvent(PlayerDeathEvent e) {
		...
		for (ItemStack i : p.getInventory().getArmorContents()) {
			if (!isAirOrNull(i)) {
				ArmorEquipEvent event = new ArmorEquipEvent(p, ArmorEquipEvent.EquipMethod.DEATH, ArmorType.matchType(i), i, null);
		...
		if (cancelled) {
			...
			}.runTaskLater(JavaPlugin.getPlugin(StrowPlugin.class), 5);
		}
	}
}
